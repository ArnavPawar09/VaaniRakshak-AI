...
print("=" * 65)
print("VaaniRakshak-AI - VaaniNet V2")
print("=" * 65)
...
def is_valid_audio(path):
    ...
    try:
        ...
        # Must contain actual samples
        if info.frames <= 0:
            return False

        # Must have a valid sample rate
...
def get_files(
    ...
    # --------------------------------------------------------
    # Use every valid file unless a limit is explicitly given
    # --------------------------------------------------------
...
def load_audio(
    ...
    if len(audio) >= TARGET_SAMPLES:
        ...
        # Short audio
        #
        # Instead of repeatedly tiling the complete waveform,
        # use reflection padding where possible.
        # This avoids creating an obvious periodic copy.
        # ----------------------------------------------------
        ...
        if len(audio) > 1:
            ...
            if training:
                ...
                # Randomly distribute the padding
                # around the original recording.
                ...
                # Deterministic padding for evaluation
                left = missing // 2
                right = missing - left
...
def main():
    ...
    for epoch in range(
        ...
        # ROC-AUC is used as a secondary tie-breaker.
        # ----------------------------------------------------
    ...
    print(
        "Loading best VaaniNet V2 model..."
    )
